		...
		<div :style='{"width":"100%","padding":"10px 10px 0","overflow":"hidden","display":"flex","height":"calc(100vh - 80px)"}'>
			<div :style='{"width":"30%","padding":"0 0px","flexWrap":"wrap","display":"flex"}'>
				<div class="tables">
					...
					<el-table :data="boardDataList" border :stripe="false" :style='{"width":"100%","padding":"0","borderColor":"rgba(0,205,215,.3)","borderStyle":"solid","borderWidth":"1px 0 0 0px","background":"none"}'>
						<el-table-column :resizable="true" :sortable="false" prop="xueli" label="学历"/>
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="jiuyedanwei" label="就业单位"/>
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="gangweixinchou" label="岗位薪酬"/>
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="value" label="数量"/>
						</el-table-column>
						<el-table-column :resizable="true" :sortable="false" prop="name" label="地区"/>
						</el-table-column>
					</el-table>
				</div>
				<div class="echarts1">
					...
					<div :style='{"width":"100%","padding":"0 16px","background":"url() no-repeat center top / 100% 100%","height":"calc(100% - 40px)"}'>
						<div id="jiuyeshujuChart1" style="width:100%;height:100%;"></div>
					</div>
				</div>
			</div>
			<div :style='{"width":"40%","padding":"0 20px","flexWrap":"wrap","display":"flex"}'>
				...
				<div :style='{"padding":"0","margin":"10px auto 20px","color":"#fff","alignItems":"center","display":"flex","justifyContent":"center","alignContent":"center","background":"none","width":"98%","fontSize":"14px","height":"130px","order":"1","zIndex":"9"}'>
					...
					<div :style='{"margin":"0 2% 0 0","alignItems":"center","background":"url(http://codegen.caihongy.cn/20231229/f20f081654fc411e841842291850c3d2.png) no-repeat center top / 100% 100%","flexDirection":"column","display":"flex","width":"18%","justifyContent":"center","height":"100%"}'>
						...
						<div :style='{"padding":"0 0px","alignItems":"center","flexDirection":"column-reverse","display":"flex"}'>
							<div :style='{"lineHeight":"32px","fontSize":"inherit","color":"inherit","height":"32px"}'>就业数据总数</div>
							<div :style='{"lineHeight":"32px","fontSize":"26px","color":"#fff","fontWeight":"bold","height":"32px"}'>{{jiuyeshujuCount}}</div>
						</div>
					</div>
				</div>
				<div :style='{"margin":"0 0 20px","color":"#fff","alignItems":"center","background":"url() no-repeat right bottom","display":"flex","width":"100%","justifyContent":"center","height":"calc(64% - 180px)","order":"2"}'>
					<div id="jiuyeshujuChart5" style="width:100%;height:100%;"></div>
				</div>
				<div class="echarts2">
					...
					<div :style='{"width":"100%","padding":"0 16px","height":"calc(100% - 40px)"}'>
						<div id="jiuyeshujuChart2" style="width:100%;height:100%;"></div>
					</div>
				</div>
			</div>
			<div :style='{"width":"30%","padding":"0 0px","flexWrap":"wrap","display":"flex"}'>
				...
				<div class="echarts3">
					...
					<div :style='{"width":"100%","padding":"0 16px","height":"calc(100% - 40px)"}'>
						<div id="jiuyeshujuChart3" style="width:100%;height:100%;"></div>
					</div>
				</div>
				<div class="echarts4">
					<div :style='{"width":"100%","padding":"0 16px","margin":"0 0 0px","textAlign":"center","background":"url(http://codegen.caihongy.cn/20240104/3d41b9dbc0284d3cbdd0e716fe2d3ed9.png) no-repeat left bottom / 100% auto","height":"40px"}'>
					...
					<div :style='{"width":"100%","padding":"0 16px","height":"calc(100% - 40px)"}'>
						<div id="jiuyeshujuChart4" style="width:100%;height:100%;"></div>
					</div>
				</div>
			</div>
		</div>
...
export default {
  ...
  created() {
    this.$nextTick(()=>{
		this.times()
		setTimeout(()=>{
			this.getjiuyeshujuCount();
			this.jiuyeshujuChat1();
			this.jiuyeshujuChat2();
			this.jiuyeshujuChat3();
			this.jiuyeshujuChat4();
			this.jiuyeshujuChat5();
		},500)
	})
  },
  methods:{
    ...
    getjiuyeshujuCount() {
        this.$http({
            url: `jiuyeshuju/count`,
            method: "get"
        }).then(({
            data
        }) => {
            if (data && data.code == 0) {
                this.jiuyeshujuCount = data.data
            }
        })
    },
...
//统计接口1
    jiuyeshujuChat1() {
      this.$nextTick(()=>{
        ...
        var jiuyeshujuChart1 = echarts.init(document.getElementById("jiuyeshujuChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                jiuyeshujuChart1.setOption(option);
				

                  //根据窗口的大小变动图表
				window.addEventListener('resize', () => {
                    jiuyeshujuChart1.resize();
                },false);
            }
        });
      })
...
//统计接口2
    jiuyeshujuChat2() {
      this.$nextTick(()=>{
        ...
        var jiuyeshujuChart2 = echarts.init(document.getElementById("jiuyeshujuChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                jiuyeshujuChart2.setOption(option);
				
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, jiuyeshujuChart2)

                  //根据窗口的大小变动图表
				window.addEventListener('resize', () => {
                    jiuyeshujuChart2.resize();
                },false);
            }
        });
      })
...
//统计接口3
    jiuyeshujuChat3() {
      this.$nextTick(()=>{
        ...
        var jiuyeshujuChart3 = echarts.init(document.getElementById("jiuyeshujuChart3"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                jiuyeshujuChart3.setOption(option);
				

                  //根据窗口的大小变动图表
				window.addEventListener('resize', () => {
                    jiuyeshujuChart3.resize();
                },false);
            }
        });
      })
...
//统计接口4
    jiuyeshujuChat4() {
      this.$nextTick(()=>{
        ...
        var jiuyeshujuChart4 = echarts.init(document.getElementById("jiuyeshujuChart4"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                jiuyeshujuChart4.setOption(option);
				

                  //根据窗口的大小变动图表
				window.addEventListener('resize', () => {
                    jiuyeshujuChart4.resize();
				},false);
            }
        });
      })
    ...
    jiuyeshujuChat5() {
		this.$nextTick(()=>{
			...
			var jiuyeshujuChart5 = echarts.init(document.getElementById("jiuyeshujuChart5"),'macarons');
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					jiuyeshujuChart5.setOption(option);
					//根据窗口的大小变动图表
					window.addEventListener('resize', () => {
						jiuyeshujuChart5.resize();
					},false);
				}
			})
		})
