    ...
    def convert_mysql_to_hive(self, raw_mysql: str) -> list:
        ...
        for parsed in sqlparse.parse(raw_mysql):
            ...
            if "insert" in method:
                ...
                table_name = ""
                # 处理提取table name失败的情况
                ret = re.findall(r'`(.*?)` \(', parsed.value,re.DOTALL)
                if len(ret) > 0:
                    table_name = f"{ret[0]}"

                if len(table_name)<1:
                    ret = re.findall(r'`(.*?)` VALUES \(', parsed.value,re.DOTALL)
                    if len(ret) > 0:
                        table_name = ret[0]
                ...
                for token in parsed.tokens:
                    ...
                    if table_name in token_value and "(" in token_value and ")" in token_value and "VALUES" not in token_value:
                        cols_string = f"{token_value}"
                        cols_string = cols_string.replace("`", "")
                    if 'VALUES' in token_value and  "'" in token_value and "(" in token_value and ")" in token_value:
                        # print(table_name,"-----", token_value, "-----")
                ...
                if len(self.db_name) > 1 and '.' not in table_name:
                    hive_insert_sql = f'''insert into {self.db_name}.{table_name} {cols_string} {values_string}'''
                else:
                    hive_insert_sql = f''' insert into {table_name} {cols_string} {values_string}'''
...
if __name__ == "__main__":
    with open(r"C:\Users\user\Desktop\python8wvnk.sql", encoding="utf-8") as f:
        data = f.read()
